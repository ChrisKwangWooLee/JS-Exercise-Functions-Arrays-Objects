...
function getCarInfoById(inventory, carId) {
  ...
  for (let i = 0; i < inventory.length; i++) {
    if (inventory[i].id === carId) {
      result = `This is a ${inventory[i].car_make} ${inventory[i].car_model}`;
      break;
    }
  }
...
function sortCarInventory(inventory) {
  let carModelArr = [];
  for (let i = 0; i < inventory.length; i++){
    carModelArr.push(inventory[i].car_model);
  }

  let sortedArr = carModelArr.sort();
  let result = [];
  
  for (let j = 0; j < sortedArr.length; j++) {
    for (let k = 0; k < inventory.length; k++) {
      if (sortedArr[j] === inventory[k].car_model){
        result[j] = inventory[k];
      }
    }
  }
